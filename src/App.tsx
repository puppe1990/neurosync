...
export default function App({
  ...
  const handleFinishPuzzle = (
    ...
    setStats((prev) => {
      ...
      const streakUpdate =
        isDaily && prev.lastChallengeDate !== todayStr
          ? prev.dailyStreak + 1
          : prev.dailyStreak;
    ...
    <div className="min-h-screen flex flex-col font-sans overflow-hidden p-4 md:p-8">
      ...
      <header className="flex justify-between items-center mb-12">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            {musicEnabled ? (
              <IconMap.Volume2 size={20} />
            ) : (
              <IconMap.VolumeX size={20} />
            )}
          </button>
              ...
              >
                <div className="bg-brand-blue border-4 border-black p-6 rounded-3xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden flex flex-col min-h-[300px]">
                  ...
                  <button
                    ...
                    onClick={() => {
                      setSelectedPuzzle(
                        PUZZLES.find((p) => p.id === 'math-rush') || PUZZLES[0],
                      );
                ...
                <div className="bg-white border-4 border-black p-5 rounded-3xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                  ...
                  <div className="flex justify-between items-end h-24 gap-2">
                    <div
                      className="bg-brand-red w-full rounded-t-lg border-2 border-black border-b-0"
                      style={{ height: '80%' }}
                    ></div>
                    <div
                      className="bg-brand-blue w-full rounded-t-lg border-2 border-black border-b-0"
                      style={{ height: '45%' }}
                    ></div>
                    <div
                      className="bg-brand-green w-full rounded-t-lg border-2 border-black border-b-0"
                      style={{ height: '95%' }}
                    ></div>
                    <div
                      className="bg-brand-gold w-full rounded-t-lg border-2 border-black border-b-0"
                      style={{ height: '60%' }}
                    ></div>
                  </div>
                </div>
          ...
          {currentView === 'TRAINING' && selectedPuzzle && (
            ...
            >
              <div className="bg-white brutalist-card p-8 mb-4">
                ...
                {selectedPuzzle.id === 'grid-memory' && (
                  <GridMemory
                    difficulty={selectedDifficulty}
                    onFinish={handleFinishPuzzle}
                  />
                )}
                {selectedPuzzle.id === 'stroop-test' && (
                  <StroopTest
                    difficulty={selectedDifficulty}
                    onFinish={handleFinishPuzzle}
                  />
                )}
                {selectedPuzzle.id === 'shape-stack' && (
                  <ShapeStack
                    difficulty={selectedDifficulty}
                    onFinish={handleFinishPuzzle}
                  />
                )}
                {selectedPuzzle.id === 'pattern-pursuit' && (
                  <PatternPursuit
                    difficulty={selectedDifficulty}
                    onFinish={handleFinishPuzzle}
                  />
                )}
                {selectedPuzzle.id === 'neural-react' && (
                  <NeuralReact onFinish={handleFinishPuzzle} />
                )}
              </div>
              <button
                onClick={() => setCurrentView('MENU')}
                ...
                <div className="divide-y-4 divide-black">
                  {stats.sessions
                    ...
                    .map((session, i) => (
                      <div
                        key={i}
                        className="p-6 flex justify-between items-center bg-white hover:bg-brand-yellow transition-colors"
                      >
                        ...
                        <div className="flex items-center gap-4">
                          <span className="text-2xl font-black italic text-brand-blue">
                            #{session.results[0].score}
                          </span>
                ...
                <div className="divide-y-4 divide-black">
                  ...
                  {!rankingsLoading &&
                    rankings.map((entry) => (
                      <div
                        key={entry.id}
                        className="p-6 bg-white hover:bg-brand-yellow transition-colors grid grid-cols-12 gap-4 items-center"
                      >
