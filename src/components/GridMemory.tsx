    ...
    <div className="flex flex-col items-center max-w-sm mx-auto">
      <div className="w-full flex justify-between items-center mb-8">
        ...
        <div className="flex flex-col items-end">
          <span className="text-[10px] font-black opacity-40 uppercase tracking-widest italic">Nível {level}</span>
      ...
      <div className="text-center mb-6 h-10 flex items-center justify-center font-black text-sm tracking-widest text-gray-900 border-b-4 border-black w-full italic uppercase">
        {gameState === 'MEMORIZE' && (
          <motion.span initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}>FOCO NO PADRÃO</motion.span>
        )}
        {gameState === 'PLAY' && (
          <motion.span initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}>RECUPERE O PADRÃO</motion.span>
        )}
        {gameState === 'FEEDBACK' && (
          <motion.span className="text-brand-red" initial={{ opacity: 0, scale: 1.2 }} animate={{ opacity: 1, scale: 1 }}>SINAL PERDIDO!</motion.span>
        )}
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 w-full aspect-square p-6 bg-white border-4 border-black rounded-3xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
        {Array.from({ length: gridSize }).map((_, i) => {
          const isHighlighted = sequence.includes(i) && (gameState === 'MEMORIZE' || gameState === 'FEEDBACK');
